...
# 入力ファイルをcp932で開き、UTF-8に変換しながら処理
open my $in_source, '<:encoding(cp932)', $input_file or die "Cannot open source.html: $!";
...
#    $content =~ s/<span class="_idGenCharOverride-.">(.)<\/span>/<span class="tcy">$1<\/span>/g;     # 縦中横英数字1文字
    $content =~ s{
            <span\s+class="_idGenCharOverride-[^"]+">\s*(.)\s*<\/span>        }{
            my $char = $1;
        # 半角英数字にマッチした場合のみ変換
            if ($char =~ /[0-9A-Za-z]/) {
...
#       ・ウムラウト等「é」
#       ・全角・半角の「’」「'」「.」「．」「，」「,」の使い分け
#       ・「Ｎｏ．１」「Ｖｏｌ.１」、「<span class="tcy">vol</span>．１」など組み合わせ

    $content =~ s/―/─/g;       # u+2015 → U+2500へ（KADOKAWA-EPUB制作仕様ver.1.3.1-20191015.pdfのp.4を参照して）
                                # これは他の線文字の対応と合わせ、変換ログ出力に「約物の変換」として含める必要あり
...
#   if ($content =~ s/([ぁ-んァ-ヶ一-龥])([0-9]{2})([ぁ-んァ-ヶ一-龥])/$1<span class="tcy">$2<\/span>$3/g) {
    if ($content =~ s/(?<=[\p{Hiragana}\p{Katakana}\p{Han}。])([0-9]{2})(?=[\p{Hiragana}\p{Katakana}\p{Han}])/<span class="tcy">$1<\/span>/g) {
...
#    <span class="tcy">2</span>　の縦中横の一桁数字をアラート出しにする？
#   $_ = '<span class="tcy">2</span>ページ目';
#    $content =~ s{<span class="tcy">([0-9])<\/span>}{chr(ord($1) + 0xFEE0)}eg;
