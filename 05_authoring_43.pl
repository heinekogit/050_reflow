...
my @annotations;
my @anno_list;
our @noteref_ids_and_filenames;
# my $noteref_ids_and_filenames;
my @noteref_ids;

# our $id_to_filename_ref;                                         # IDからファイル名へのマッピング
#    my @toc_id_to_filename = ();                            # ここで宣言する
#    my $toc_id_to_filename_ref = \@toc_id_to_filename;                  # リファレンスを作成
...
#    shosi.csvの読み込み部分   	 ===========================================================================
    ...
    # 書誌の設定に則り、各サブルーチンを実行していく
    foreach(@shosi)   	 
   	 {
   		@koumoku_content = split(/,/);
        my @id_to_filename = ();        # ループごとにリセット
		...
		&make_page_colophon;		# 奥付
   		...
   		&make_xhtml;   				        # xhtmlの切り出し分割
			# |_ &write_to_file             # 各xhtmlファイルの中身整形サブルーチン。		
		        # |_ make_page_annotation	# 各xhtmlの注釈が単ページの場合の分岐サブルーチン。	 
        ...
        # （機能目次）navigationの生成
		make_file_navigation (\@id_to_filename);

        # （目次ページ）tocの生成
        if ($koumoku_content[5] eq 'yes') {
		    make_file_toc(\@id_to_filename);
        } else {                            # 目次がない場合
            # コピー元とコピー先のファイルパスを定義
            my $source_file = "05_assemble/navigation-documents_no_toc.xhtml";
            my $destination_file = "06_output/$koumoku_content[3]/item/navigation-documents.xhtml";
            # ファイルをコピー
...
sub make_xhtml {
    # 素材htmlを読み込み
    open(my $input_fh, '<:encoding(UTF-8)', "05_assemble/target.xhtml") or die "Cannot open file 05_assemble/target.xhtml: $!";

    # toc_id_to_filename_ref が未定義なら、空の配列リファレンスをセット
    $toc_id_to_filename_ref //= [];
    ...
    while (my $line = <$input_fh>) {
        ...
        if (defined $line && $line =~ m/<div class="_idGen[^"]*">/) {
            if ($copying) {
                # 余分な空行を削除
...
#   逆順テスト終り　---------------------------------------------------------------------------------------------------
                ...
                if (defined $line && $line =~ m/<div class="_idGenObjectLayout-[^"]*">/) {
                    $current_template = "00_templates/p-gazou.xhtml";
#                    print "Selected template: p-gazou.xhtml for line: $line\n";  # デバッグ用出力
                } elsif (defined $line && $line =~ m/<div class="_idGenStoryDirection-[^"]*">/) {
                    $current_template = "00_templates/p-text.xhtml";
#                    print "Selected template: p-text.xhtml for line: $line\n";  # デバッグ用出力
                } else {
                    # 条件に一致しなかった場合の処理
                    print "Warning: No matching template for line: $line\n";
                    $current_template = "00_templates/p-text.xhtml";  # デフォルトのテンプレートを設定
                }
            # コピーを開始する
            $copying = 1;
            @lines_to_copy = ();  # 配列をクリア
...
sub write_to_file {
    my ($file_counter, $lines_ref, $template_fh, $filenames_ref) = @_;      # $lines_ref は、@lines_to_copy（本文？）
                                                                            # $template_fh は書き込み用テンプレファイル
                                                                            # $filenames_ref は、@output_filenames（xhtml名リスト）
    # 出力ファイルのパスを作成
    my $filename = sprintf("06_output/%s/item/xhtml/p-%03d.xhtml", $koumoku_content[3], $file_counter);
    open(my $output_fh, '>:encoding(UTF-8)', $filename) or die "Could not open output file '$filename': $!";

    # ファイル名をリストに追加
    push @$filenames_ref, $filename;

    my $current_index = sprintf("p-%03d.xhtml", $file_counter);  # 3桁にフォーマット
    my @current_files;
    our @noteref_ids_and_filenames;

    push @current_files, $current_index;  # 現在のXHTMLファイル名を記録
    ...
    # 注釈リンク配列の作成 -------------------------------------------------------------------------------
    my $anno_content_to_insert = join("\n", @$lines_ref);

#    $anno_content_to_insert =~ s{href="target\.xhtml(#[^"]*)?"}{href="$current_index$1"}g;          # href 属性の target を置換

    # `noteref` ID と `$current_index` のペアを配列に追加
    while ($anno_content_to_insert =~ /<a class="noteref" href="target.xhtml#note-(\d{3})"/g) {
        my $noteref_id = $1;  # キャプチャされた三桁の数値

        # IDが空でない場合のみ配列に追加
        if (defined $noteref_id && $noteref_id ne '') {
            push @noteref_ids_and_filenames, { id => $noteref_id, filename => $current_index };
#            print "annotation: id $noteref_id Filename: $current_index\n";                      # デバッグ用
        } else {
            warn "id nothing\n"; 
        }
    }   
#   以下デバッグ用
#    print "Debug:(脚注_配列作成の登録確認):\n";
#    print Dumper(@noteref_ids_and_filenames);
#   配列 @noteref_ids_and_filenames の内容を表示
#    print "Noteref IDs and Filenames:\n";
#    foreach my $entry (@noteref_ids_and_filenames) {
#        print "annotation: $entry->{id} => Filename: $entry->{filename}\n";
#    }
    ...
    # toc_id の取得
    my $found_id = '';
#    if (my $content_to_insert =~ /<a id="(toc-\d+)"><\/a>/) {
    if ($content_to_insert =~ /<a id="(toc-\d+)"><\/a>/) {

        $found_id = $1;
        if (defined $found_id && defined $filename) {
            push @$toc_id_to_filename_ref, { id => $found_id, filename => $current_index };
#            warn "Pushed: id=$found_id, filename=$current_index";                              # デバッグ
           }
        }

#           デバッグ
#            warn "Current TOC List: ", Dumper($toc_id_to_filename_ref);
#            print "Debug:(目次_配列作成時のxhtmlファイル名の確認):\n";
#            print Dumper($toc_id_to_filename_ref);  # これでリファレンスの中身を表示します。

    # 目次リンクの処理 終り -------------------------------------------------------------------------------

    # 注釈リンクがリンク先と同ページである場合 ---------------------------------------------------------------
    # 本文中noteのidを検索し、紐づいた分割xhtmlファイル名に置き換え
    if ($koumoku_content[6] eq 'no') {

        # 本文中の注釈リンクを置換
        $content_to_insert =~ s{href="target\.xhtml#(noteref-\d{3}|note-\d{3})"}{
            my $note_id = $1;
    my $original_id = $1;  # 元のIDを保持
            $note_id =~ s/^(noteref|note)-//;  # プレフィックスを除去
            $note_id = sprintf("%03d", $note_id);  # ゼロ埋め（必要に応じて）

#            print "note_id ha " . $note_id . " \n";                            # デバッグ用

            # id_to_filename でファイル名を取得（正しいマッチング）
            my $replacement_file = '';
            for my $entry (@noteref_ids_and_filenames) {
                if ($entry->{id} eq $note_id) {
                    $replacement_file = $entry->{filename};
                last;
                }
            }

        # もしファイル名が見つからない場合のデフォルト処理
        $replacement_file = 'target.xhtml' if $replacement_file eq '';

#        print "replace ha " . $replacement_file . "\n";                         # デバッグ用
#        print "Replacing target.xhtml# with href=\"$replacement_file#$1\"\n";   # デバッグ用

        "href=\"$replacement_file#$original_id\"";
        }ge;

    } elsif ($koumoku_content[6] eq 'yes') {                                                # 単体の注釈ページがアリの場合
            $content_to_insert =~ s/href="target\.xhtml#/href="p-annotation\.xhtml#/g;      # 本文からのリンク先は p-annotation.xhtml の一択。
            make_page_annotation(\@noteref_ids_and_filenames);
    }
    ...
    foreach my $temp_line (@template_lines) {
        ...
        $temp_line =~ s|<a id="toc-\d+">(.*?)</a>|$1|g;         # 本文中のリンク先の記述タグを消す（25/3/10 GPTに消されていたもの復帰）
        $temp_line =~ s/<div class="_idGen(.*?)">\n//;            # <div class="_idGenObject[^"]*">を消す（25/3/10 GPTに消されていたもの復帰）
...
#    return $filename;
    return ($filename, \@current_files, \@noteref_ids);
}
...
#   	 rcopy("00_templates/item/navigation-documents.xhtml","06_output/$koumoku_content[3]/item") or die "styleをコピーできません\n";
  	 ...
  	 #    shosi.csvを生成xhtml階層にログ的コピー保存    -----------    
   	 ...
   	 rcopy("05_assemble/go_annotation.xhtml","06_output/used_sozai_$koumoku_content[3]") or die "go_annotation.xhtmlを履歴用にコピーできません\n";
   	 rcopy("05_assemble/go_colophon.xhtml","06_output/used_sozai_$koumoku_content[3]") or die "go_colophon.xhtmlを履歴用にコピーできません\n";
...
sub make_file_navigation {
    my (@toc_id_to_filename_ref) = @_;
    
    # 目次データを読み込む
    open(my $nav_fh, '<:encoding(UTF-8)',  "05_assemble/go_navigation.xhtml") or die "Cannot open file go_navigation.xhtml: $!";
    my @nav_lines = <$nav_fh>;
    close($nav_fh);

    # テンプレートファイルを読み込む
    open(my $template_fh, '<:encoding(UTF-8)', "00_templates/navigation-documents.xhtml") or die "Cannot open file navigation-documents.xhtml: $!";
    my @template_lines = <$template_fh>;
    close($template_fh);

#        print "Debug:(naviファイル出力時の目次とxhtmlファイル名の確認):\n";
#        print Dumper($toc_id_to_filename_ref);  # これでリファレンスの中身を表示します。

    # 目次リストを生成
    my @nav_list;
    for my $line (@nav_lines) {
        next unless defined $line;          # $line が undef の場合はスキップ
        chomp $line;

        # デバッグ
#            print "Line: $line\n";

        if ($line =~ m/target.xhtml#(toc-\d+)/) {
            my $toc_id = $1;
            # デバッグ
            # print "$toc_id\n";
            my ($id_entry) = grep { $_->{id} eq $toc_id } @$toc_id_to_filename_ref;
            if ($id_entry) {
                my $filename = $id_entry->{filename};
                $filename =~ s|^.*?/item/||;                            # フォルダ名を除去
                $line =~ s/href="target\.xhtml/href="xhtml\/$filename/;
            }   
        }
...
#        $line =~ s/<li>//;
    ...
    open(my $output_fh, '>:encoding(UTF-8)', "06_output/$koumoku_content[3]/item/navigation-documents.xhtml") or die "Could not open navigation file: $!";
    for my $temp_line (@template_lines) {
        chomp $temp_line;
        $temp_line =~ s/$placeholder/$nav_content/;
        print $output_fh "$temp_line\n";
    }
    close($output_fh);
}

# toc（p-toc.xhtml）生成・出力サブルーチン ===========================================================================

sub make_file_toc {
    my (@toc_id_to_filename_ref) = @_;
    
    # 目次データを読み込む
    open(my $toc_fh, '<:encoding(UTF-8)', "05_assemble/go_toc.xhtml") or die "Cannot open file toc_data: $!";
    my @toc_lines = <$toc_fh>;
    close($toc_fh);

    # テンプレートファイルを読み込む
    open(my $template_fh, '<:encoding(UTF-8)', "00_templates/p-toc.xhtml") or die "Cannot open file toc template: $!";
    my @template_lines = <$template_fh>;
    close($template_fh);

#        print "Debug:(tocファイル出力時のxhtmlファイル名の登録確認):\n";
#        print Dumper($toc_id_to_filename_ref);  # これでリファレンスの中身を表示します。

    # 目次リストを生成 --------------------------------------------------------------------------
    my @toc_list;
    for my $line (@toc_lines) {
        chomp $line;
    ...
    # テンプレートファイルの内容を読み込み、プレースホルダーを置換して出力ファイルに書き込む
    open(my $output_fh, '>:encoding(UTF-8)', "06_output/$koumoku_content[3]/item/xhtml/p-toc.xhtml") or die "Could not open output toc: $!";
    for my $temp_line (@template_lines) {
        chomp $temp_line;
...
# 単ページのannotation（p-annotation.xhtml）を生成・出力するサブルーチン =======================================================

sub make_page_annotation {
    my ($anno_id_to_filename_ref) = @_;

    # デバッグ用に @anno_id_to_filename_ref の内容を出力
#    print "anno_id_to_filename_ref:\n";
#    print Dumper($anno_id_to_filename_ref);

    # 脚注データを読み込む
    open(my $annot_fh, '<:encoding(UTF-8)', "05_assemble/go_annotation.xhtml") or die "Cannot open file 05_assemble/go_annotation.xhtml: $!";
    my @anno_lines = <$annot_fh>;
    close($annot_fh);

 # 空行を削除
    @anno_lines = grep { $_ !~ /^\s*$/ } @anno_lines;
    # 各行の末尾の改行を削除
    chomp @anno_lines;

    # テンプレートファイルを読み込む
    open(my $template_fh, '<:encoding(UTF-8)', "00_templates/p-annotation.xhtml") or die "Cannot open file 00_templates/p-annotation.xhtml: $!";
    my @template_lines = <$template_fh>;
    close($template_fh);

    # デバッグ用に確認
    # print $id_to_filename{"note-001"} . "\n";  # 例: note-001 に対応するファイル名を表示

    # annotation文から 配列の `id` を検索、マッチしたら適切な xhtml ファイル名に置き換え
    for my $line (@anno_lines) {
        # href のマッチを確認
        if ($line =~ /href="target\.xhtml#(noteref-\d+)"/) {
            my $note_id = $1;                                       # 例: noteref-001
            $note_id =~ s/^noteref-//;                              # "noteref-" を取り除く
            my $num_id = int($note_id);                             # 数字部分を整数として変換
            $note_id = sprintf("%03d", $num_id);                    # 数字を三桁でゼロ埋め
#            print "Found note_id: $note_id\n";                      # note_id をデバッグ出力

            # @anno_id_to_filename_ref から対応するファイル名を取得
            my ($entry) = grep { $_->{id} eq $note_id } @$anno_id_to_filename_ref;
            if ($entry) {
                my $replacement_file = $entry->{filename};
#                print "Before replacement: $line\n";                # デバッグ 置き換え前の行を表示
                $line =~ s{href="target\.xhtml}{href="$replacement_file}g;
#                print "After replacement: $line\n";                 # デバッグ 置き換え後の行を表示
            } else {
                print "No match for ID: $note_id\n";  # マッチしなかったIDを表示
            }
        }
    }
    ...
    # `@anno_lines` を `join` して `anno_content` を作成
    my $anno_content = join("\n", @anno_lines);
    my $placeholder = quotemeta('▼注釈コンテンツ挿入位置▼');
    
    # テンプレートファイルの内容を読み込み、プレースホルダーを置換して出力ファイルに書き込む
    open(my $output_fh, '>:encoding(UTF-8)', "06_output/$koumoku_content[3]/item/xhtml/p-annotation.xhtml") or die "Could not open output file 'output_anno': $!";
    for my $temp_line (@template_lines) {
#        chomp $temp_line;
