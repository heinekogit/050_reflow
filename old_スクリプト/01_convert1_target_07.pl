...
my $output_file3 = "02_getset/convert.log";             # 出力ログ
...
binmode(STDOUT, ":encoding(UTF-8)");
    ...
    # 正規表現による変換 + ログ出力処理
    while ($content =~ /([ぁ-ん一-龥])([0-9]{2})([ぁ-ん一-龥])/) {
        my $match = $1 . $2 . $3;                                # 変換後の文字列
#        print "Match found: " . $match . "\n";  # 各部分を表示
        print "Match found: " . encode('UTF-8', $match) . "\n";  # UTF-8にエンコードして出力

#        my $match = "$1<span class=\"tcy\">$2<\/span>$3";  # 変換後の文字列

        # マッチした位置の前後10文字を取得
        my $start = $-[0] - 10;
        my $end   = $+[0] + 10;
        $start = 0 if $start < 0;
        # 文字単位で前後を取得するために文字列として扱う
        my $utf8_content = decode('UTF-8', $content);  # UTF-8としてデコード
        $context = substr($utf8_content, $start, $end - $start);  # 正しく文字単位で切り出し
#       $context = $1;
    }

# ログ出力
#    open my $out_log, '>', $output_file3 or die "Cannot open $output_file3: $!";
    open my $out_log, '>:encoding(UTF-8)', $output_file3 or die "Cannot open $output_file3: $!";

    my $log_message = "【二桁英数字にtcyを付加】\n";
    print $out_log $log_message;
    print $out_log "$context\n";
