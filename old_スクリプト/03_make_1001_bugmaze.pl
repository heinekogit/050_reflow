...
# 		（機能目次）navigationの生成
		update_navigation(\@id_to_filename, "01_templates/navigation-documents.xhtml", "04_output/$koumoku_content[4]/item/navigation-documents.xhtml", "03_materials/go_navigation.xhtml");

# 		（目次ページ）tocの生成
		update_toc(\@id_to_filename, "01_templates/p-toc.xhtml", "04_output/$koumoku_content[4]/item/xhtml/p-toc.xhtml", "03_materials/go_toc.xhtml");
...
#   	&output_txts;   					#prcs07		テキスト類の出力
   	 ...
   	 open(LOGS, ">:encoding(UTF-8)", "04_output/used_sozai_$koumoku_content[4]/log.txt") or die "cant open log_file\n";   	 #002以降xhtmlファイルの出力
   	 print LOGS @log;
   	 close(LOGS);
...
# </div> 削除前の確認処理
if (@lines_to_copy && $lines_to_copy[-1] =~ m|</div>|) {
#    print "Found </div> tag, checking previous lines for important content.\n";
    # ここで必要に応じてIDが含まれているかをチェック
    if ($lines_to_copy[-2] =~ m|id="([^"]+)"|) {
        print "Found ID: $1, not removing associated line.\n";
        # 必要であれば削除せずに次に進む
    } else {
#        print "Removing </div> tag: $lines_to_copy[-1]\n";
        pop @lines_to_copy;
    }
}

    # デバッグ用に @lines_to_copy の内容を再表示
#    print "After removal, @lines_to_copy contains:\n";
...
# デバッグ: 削除前の逆順配列をログ出力
#   print "削除前の逆順配列: @reversed_lines\n";

# 逆順配列から最初の </div> タグを削除
if (@reversed_lines && $reversed_lines[0] =~ m|</div>|) {
    shift @reversed_lines;  # 最初の要素 (</div> タグ) を削除
}

# デバッグ: 削除後の逆順配列をログ出力
#   print "削除後の逆順配列: @reversed_lines\n";

# 配列を元の順序に戻す
@lines_to_copy = reverse @reversed_lines;

# デバッグ: ファイル書き込み前の最終配列をログ出力
#   print "処理後の lines_to_copy: @lines_to_copy\n";
...
sub write_to_file {
    ...
    # 本体コンテンツ内で<a id="toc-001"></a>を検索						
    if ($content_to_insert =~ /<a id="(toc-\d+)"><\/a>/) {
        $found_id = $1;

    # デバッグ用ログ出力
#    print "Captured ID: $found_id, Filename: $filename\n";                      #   0905朝、ココOK
    ...
    # デバッグ情報の出力
#    print "Debug: Current ID to Filename Mapping:\n";
#    for my $entry (@$id_to_filename_ref) {
#        print "in_ref_ID: " . $entry->{id} . ", Filename: " . $entry->{filename} . "\n";    #   0905朝、ココOK
#    }
...
#   	 $koumoku_name[4];   							 #話のファイル名
   	 ...
   	 mkdir("04_output/used_sozai_$koumoku_content[4]", 0755) or die "使用済データのフォルダを作成できませんでした\n";
...
#   	 rcopy("01_templates/item/navigation-documents.xhtml","04_output/$koumoku_content[4]/item") or die "styleをコピーできません\n";
  	 ...
  	 #    画像ファイルコピー    -----------    
   	 ...
   	 rcopy("03_materials/image_fixity","04_output/$koumoku_content[4]/item/image") or die "$koumoku_content[4]の固定画像をコピーできません\n";
  	 ...
  	 #    shosi.csvを生成xhtml階層にログ的コピー保存    -----------    
   	 ...
   	 rcopy("03_materials/shosi.csv","04_output/used_sozai_$koumoku_content[4]") or die "shosiを履歴用にコピーできません\n";
   	 rcopy("03_materials/go_navigation.xhtml","04_output/used_sozai_$koumoku_content[4]") or die "shosiを履歴用にコピーできません\n";
   	 rcopy("03_materials/go_toc.xhtml","04_output/used_sozai_$koumoku_content[4]") or die "shosiを履歴用にコピーできません\n";
   	 rcopy("03_materials/target.html","04_output/used_sozai_$koumoku_content[4]") or die "shosiを履歴用にコピーできません\n";
...
#	カバー画像のサイズを取得    ------------------------------------
   	 ...
   	 my $zeroone = glob("04_output/$koumoku_content[4]/item/image/i-cover.jpg");   				#
   	 # .jpg のサイズを取得
   		 (my $cover_width, my $cover_height) = imgsize("04_output/$koumoku_content[4]/item/image/i-cover.jpg");		#パターンa	001を直で指定	イキ
...
#   03_materials/go_navigation.xhtml                                    #  ①これを取り込み、
#   01_templates/navigation-documents.xhtml                             #  ②貼り付け台に                        
#   04_output/$koumoku_content[4]/item/navigation-documents.xhtml       #  ③目次データ出力

sub update_navigation {
    my ($id_to_filename_ref, $template_path, $output_path, $navigation_data_path) = @_;

    # デバッグ用: IDとファイル名のリストを出力
#    foreach my $entry (@$id_to_filename_ref) {
...
#        chomp $line;
        if ($line =~ m/href="target\.html#(toc-\d+)"/) {
            my $toc_id = $1;
            my ($id_entry) = grep { $_->{id} eq $toc_id } @$id_to_filename_ref;

        #　デバッグ出力
#           print "ID_ENTRY: " . (defined $id_entry ? "ID: $id_entry->{id}, Filename: $id_entry->{filename}" : "None") . "\n";

            if ($id_entry) {
                my $filename = $id_entry->{filename};
                $filename =~ s|^.*?/item/||;                    # フォルダ名を除去
                $line =~ s/href="target\.html/href="$filename/;

            # デバッグ用ログ出力
#                print "Replacing_1: href=\"target.html#$toc_id\" with href=\"$filename\"\n";
#            } else {
#                # $id_entry が見つからない場合のデバッグ出力
#                print "ID not found: $toc_id\n";
#            }
...
#   デバッグ
#            print "Replacing_2: href=\"target.html#$toc_id\" with href=\"$id_entry->{filename}\"\n"; 

                my $filename = $id_entry->{filename};
                $filename =~ s|^.*?/item\/xhtml/||;         # フォルダ名を除去
                $line =~ s/href="target\.html/href="$filename/;
                } else {                                                    #  デバッグで付加
                    print "No match found for ID: $toc_id\n";               #  デバッグで付加        
                 }                                                          #  デバッグで付加
        }
        push @toc_list, $line;
    }
