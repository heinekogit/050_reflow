...
# 		（機能目次）navigationの生成
		update_navigation(\@id_to_filename, "00_templates/navigation-documents.xhtml", "04_output/$koumoku_content[4]/item/navigation-documents.xhtml", "03_materials/go_navigation.xhtml");

# 		（目次ページ）tocの生成
		update_toc(\@id_to_filename, "00_templates/p-toc.xhtml", "04_output/$koumoku_content[4]/item/xhtml/p-toc.xhtml", "03_materials/go_toc.xhtml");
...
sub write_to_file {
    ...
    while (my $temp_line = <$template_fh>) {
        ...
        $temp_line =~ s/<div class="_idGen(.*?)"> <!—改ページ位置 --------------------------------------------->\n//;            # <div class="_idGenObject[^"]*">を消す
...
#   	 $koumoku_name[4];   							 #話のファイル名
   	 ...
   	 rcopy("00_templates/META-INF/container.xml","04_output/$koumoku_content[4]/META-INF") or die "container.xmlをコピーできません\n";
   	 rcopy("00_templates/mimetype","04_output/$koumoku_content[4]") or die "mimetypeをコピーできません\n";
   	 rcopy("00_templates/style","04_output/$koumoku_content[4]/item/style") or die "styleをコピーできません\n";
#   	 rcopy("00_templates/item/navigation-documents.xhtml","04_output/$koumoku_content[4]/item") or die "styleをコピーできません\n";
...
# 奥付用を読み込む
open(GO_COLOPHON, '<:encoding(UTF-8)', "03_materials/go_colophon.xhtml") or die "Cannot open go_colophon: $!";
