...
sub write_to_file {
    ...
    # `noteref` や `note` の ID を検索し、対応するXHTMLファイルを記録
    foreach my $entry (@$id_to_filename_ref) {
        my $id = $entry->{id};               # ID (e.g., note-050, noteref-050)
        my $file = $entry->{filename};       # 分割XHTMLのファイル名 (e.g., p-003.xhtml)

        # `target.xhtml` を正しいXHTMLファイル名に置換
        $content_to_insert =~ s/href="target\.xhtml#$id"/href="$file#$id"/g;
    }

    # **追加: `noteref-XXX` の ID に対応するXHTMLを検索**
    while ($content_to_insert =~ /href="target\.xhtml#(noteref-\d+)"/g) {
        my $id = $1;
        my ($matching_entry) = grep { $_->{id} eq $id } @$id_to_filename_ref;
        if ($matching_entry) {
            my $new_file = $matching_entry->{filename};
            $content_to_insert =~ s/href="target\.xhtml#$id"/href="$new_file#$id"/g;
        }
    }
    ...
    while (my $temp_line = <$template_fh>) {
        ...
        $temp_line =~ s/●タイトル名●/$koumoku_content[0]/;  # 標準的な文字置き換え

        # _idGenStoryDirection-2 に応じて ●方向● の切り替え
        if ($temp_line =~ /●方向●/) {
            if ($content_to_insert =~ m/<div class="_idGenStoryDirection-2">/) {
        ...
        # 本文中のリンク先の記述タグを消す
        $temp_line =~ s|<a id="toc-\d+">(.*?)</a>|$1|g;

        # <div class="_idGenObject[^"]*"> を削除
