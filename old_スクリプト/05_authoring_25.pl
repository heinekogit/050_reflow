...
sub make_xhtml {
    ...
    my @lines_to_copy;
    my $current_template = '';
    my @output_filenames = ();      # 出力ファイル名配列
    my %id_to_filename = ();        # IDとファイル名の対応
...
#   逆順テスト終り　---------------------------------------------------------------------------------------------------
    ...
    my ($file_counter, $lines_to_copy_ref, $current_template, $output_filenames_ref, $id_to_filename_ref) = @_;

    # lines_to_copyが定義されているか確認
    unless (defined $lines_to_copy_ref && @$lines_to_copy_ref) {
        die "Error: lines_to_copy_ref is undefined or empty.";
    }

    # 注釈リンクの整形処理を呼び出す
    process_annotations($lines_to_copy_ref, $output_filenames_ref, $id_to_filename_ref);

    open(my $template_fh, '<:encoding(UTF-8)', $current_template) or die "Cannot open file $current_template: $!";
#    write_to_file($file_counter, \@lines_to_copy, $template_fh, \@output_filenames);
    write_to_file($file_counter, $lines_to_copy_ref, $template_fh, $output_filenames_ref, $id_to_filename_ref);    close($template_fh);
...
sub process_annotations {
    my ($lines_ref, $output_filenames_ref, $id_to_filename_ref) = @_;

    # 注釈リンクの置き換え処理
    foreach my $line (@$lines_ref) {
        # 注釈リンクを置き換える
        if ($line =~ /<a class="noteref" href="([^"]+)"/) {
            my $href = $1;
            if ($href =~ /target.xhtml#note-(\d+)/) {
                my $note_id = $1;
                my $new_filename = $id_to_filename_ref->{$note_id} || 'default.xhtml';
                $line =~ s/$href/"$new_filename#note-$note_id"/g;
            }
        }
        if ($line =~ /<p class="note"><a class="note" href="([^"]+)"/) {
            my $href = $1;
            if ($href =~ /target.xhtml#note-(\d+)/) {
                my $note_id = $1;
                my $new_filename = $id_to_filename_ref->{$note_id} || 'default.xhtml';
