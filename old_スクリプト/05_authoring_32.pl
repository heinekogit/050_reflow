...
#    my %id_to_filename;             # IDからファイル名へのマッピング
    ...
    # 脚注リンクのIDと対応する分割ページファイル連番を格納する配列
    my %footnote_links_to_pages;
...
while ($content_to_insert =~ /<a class="noteref" href="adjust_target\.xhtml#note-\d+" id="noteref-(\d{3})"/g) {
#        print "Found noteref id: $1\n";         # ID が一致したかどうか確認
    my $noteref_id = $1;    # キャプチャされた三桁の数値
    push @noteref_ids_and_filenames, { id => $noteref_id, filename => $current_index };  
                            # ID と filename のペアをハッシュリファレンスとして配列に追加
}
...
sub make_page_annotation {
    ...
    for my $line (@anno_lines) {
        # <a class="note" href="adjust_target.xhtml#noteref-001" id="note-001"> 形式にマッチ
        if ($line =~ /<a class="note" href="adjust_target\.xhtml#(noteref-\d+)" id="(note-\d+)">/) {
            my $note_id = $2;  # id="note-001" の形式でキャプチャした note の id
            if (exists $id_to_filename{$note_id}) {
                my $replacement_file = $id_to_filename{$note_id};  # 対応する分割xhtmlファイル名
                # href を置き換え
