...
sub write_to_file {
    ...
    while ($content_to_insert =~ /<a class="noteref" href="adjust_target\.xhtml#note-(\d{3})"/g) {
...
for my $line (@anno_lines) {
    # href のマッチを確認
if ($line =~ /href="adjust_target\.xhtml#(noteref-\d+)"/) {
...
print "Checking: $note_id\n";
        ...
        $line =~ s{href="adjust_target\.xhtml}{href="$replacement_file}g;
