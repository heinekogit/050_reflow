...
sub update_navigation {
    ...
    if (!@nav_lines) {
        die "Error: @nav_lines is empty or undefined\n";
        }
    ...
    for my $line (@nav_lines) {
        ...
        # デバッグ
            print "Line: $line\n";
            print Dumper($id_to_filename_ref);
#        if ($line =~ m/href="target\.xhtml#(toc-\d+)"/) {      # オリジナル
#        if ($line =~ m/<li><a href="target\.xhtml#(toc-\d+)">/) {
#        if ($line =~ m/<a href="target\.xhtml#(toc-\d+)">/gi) {
#        if ($line =~ m/<a\s+href="target\.xhtml#(toc-\d+)"/gi) {
#    if ($line =~ m/<a\s+href=(?:"|&quot;)target\.xhtml#(toc-\d+)(?:"|&quot;)>/gi) {
#    if ($line =~ m/<a\s+href=(?:"|&quot;)target.xhtml#(toc-...)(?:"|&quot;)>/gi) {
#    if ($line =~ m/href=(?:"|&quot;)target\.xhtml#(toc-\d+)(?:"|&quot;)/) {

#    my $bun = "<a href="target.xhtml\#";"
#    if ($line =~ m/$bun(toc-\d+)/) {
...
sub make_page_annotation {
    ...
    my %id_to_filename = map { $_->{id} => $_->{filename} } @$id_to_filename_ref;
...
# $id_to_filename_ref の内容をデバッグ出力
print "id_to_filename_ref:\n";
print Dumper($id_to_filename_ref);
