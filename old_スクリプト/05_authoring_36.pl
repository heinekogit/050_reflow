...
sub write_to_file {
    my ($counter, $lines_ref, $template_fh, $filenames_ref, $toc_id_to_filename_ref, $koumoku_content) = @_;
    ...
    while ($content_to_insert =~ /<a class="noteref" href="target\.xhtml#note-(\d{3})"/g) {
...
#    print "hairetu ----- Noteref IDs and Filenames:\n";
#    foreach my $entry (@noteref_ids_and_filenames) {
#        print "ID: $entry->{id} => Filename: $entry->{filename}\n";
#    }
...
#     $toc_id_to_filename_ref //= [];
    ...
    if ($content_to_insert =~ /<a id="(toc-\d+)"><\/a>/) {
        $found_id = $1;
#        push @$toc_id_to_filename_ref, { id => $found_id, filename => $filename };     # オリジナル
    if (defined $found_id && defined $filename) {
        push @$toc_id_to_filename_ref, { id => $found_id, filename => $filename };
        warn "Pushed: id=$found_id, filename=$filename";}
    }
        # デバッグ
...
sub update_navigation {
    my (@toc_id_to_filename_ref, $template_path, $output_path, $navigation_data_path) = @_;
...
sub update_toc {
    my (@toc_id_to_filename_ref, $template_path, $output_path, $toc_data_path) = @_;
    
    # 目次データを読み込む
    open(my $toc_fh, '<:encoding(UTF-8)', $toc_data_path) or die "Cannot open file $toc_data_path: $!";
    my @toc_lines = <$toc_fh>;
    close($toc_fh);
...
# annotation（p-annotation.xhtml）生成・出力をするサブルーチン =======================================================
