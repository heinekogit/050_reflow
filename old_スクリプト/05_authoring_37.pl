    ...
    foreach(@shosi)   	 
		...
		&make_page_colophon;		# 奥付
   		...
   		&make_xhtml;   				# prcs04		xhtmlのセッティング・出力
			# |_ &write_to_file     # このサブルーチンは make_xhtml 内で定義されている		
...
sub make_page_annotation {
    my ($anno_id_to_filename_ref, $template_path, $output_path, $annotation_data_path) = @_;
...
#        print "Debug:(脚注_出力時のxhtmlファイル名の登録確認):\n";
#        print Dumper($anno_id_to_filename_ref);  # これでリファレンスの中身を表示します。

    # 本文中noteのidを検索し、紐づいた分割xhtmlファイル名に置き換え ---------------------------------
    # @noteref_ids_and_filenames 配列をもとに %id_to_filename を作成
    my %id_to_filename = map { $_->{id} => $_->{filename} } @$anno_id_to_filename_ref;
    # デバッグ用に %id_to_filename の内容を出力
        print "ID to Filename Mapping:\n";
        for my $id (keys %id_to_filename) {
            print "ID: $id => Filename: $id_to_filename{$id}\n";
        }

# $anno_id_to_filename_ref の内容をデバッグ出力
...
# @noteref_ids_and_filenames 配列をもとに %id_to_filename を作成
    %id_to_filename = map { $_->{id} => $_->{filename} } @noteref_ids_and_filenames;

    print "ID to Filename Mapping:\n";
foreach my $id (keys %id_to_filename) {
    print "ID: $id -> Filename: $id_to_filename{$id}\n";
}
...
# 本文中の `note` の `id` を検索し、適切な xhtml ファイルに置き換え
...
if ($line =~ /href="target\.xhtml#(noteref-\d+)"/) {
    ...
    if (exists $id_to_filename{$note_id}) {
        print Dumper(\%id_to_filename);
        print "Checking: $note_id\n";
        print "yes\n";

        my $replacement_file = $id_to_filename{$note_id};
